<script type="text/javascript">
    ...
    $('#gol_bimwin').select2({
        ...
        dropdownParent: $('#tambahBimwin') // 🔥 INI KUNCI
    });

    $(document).ready(function () {
        $('#bimwinTable').DataTable({
            fixedColumns: {
                left: 1,
                right: 1
            },
    ...
    Page.Batal = function()
    {
        $.ajax({
            type: 'GET',
            beforeSend: function() {
                $('#tambahBimwin').modal('hide');
                $('#detailBimwin').modal('hide');
                $('#editBimwin').modal('hide');
                $('#importBimwin').modal('hide');
                $('#Loader').modal('show');
            },
                ...
                success: function(response){
                    ...
                    if (validGolongan.includes(response.data.golongan)) {
                        ...
                        $('#e_gol_bimwin').select2({
                            ...
                            placeholder: 'Pilih',
                            minimumResultsForSearch: 0,
                            dropdownParent: $('#editBimwin') // 🔥 INI KUNCI
                        });
                    }
        ...
        const form = document.getElementById("form_edit_bimwin_peserta");
            ...
            try {
                ...
                if (result.message === 200) {
                    ...
                    Swal.fire("Error", "Maaf, tidak bisa diperbaharui jika beda kegiatan, nik dan nip.", "error").then(() => {
                        setTimeout(() => {
                            showModal("editBimwin");
