<script type="text/javascript">
    ...
    $('#gol_bimwin').select2({
        ...
        dropdownParent: $('#tambahBrus') // 🔥 INI KUNCI
    });
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var i = 0; i < response.data.length; i++) {
                ...
                if (item.kategori != 452) continue; // brus
    ...
    Page.Batal = function()
    {
        $.ajax({
            type: 'GET',
            beforeSend: function() {
                $('#tambahBrus').modal('hide');
                $('#detailBrus').modal('hide');
                $('#editBrus').modal('hide');
                $('#importBrus').modal('hide');
                $('#Loader').modal('show');
            },
                ...
                success: function(response){
                    ...
                    if (validGolongan.includes(response.data.golongan)) {
                        ...
                        $('#e_gol_bimwin').select2({
                            ...
                            placeholder: 'Pilih',
                            minimumResultsForSearch: 0,
                            dropdownParent: $('#editBrus') // 🔥 INI KUNCI
                        });
                    }
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        form.addEventListener("submit", function (e) {
            ...
            const modalEl = document.getElementById("editBrus");
