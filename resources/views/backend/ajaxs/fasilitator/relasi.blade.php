<script type="text/javascript">
    ...
    $('#gol_bimwin').select2({
        ...
        dropdownParent: $('#tambahRelasi') // 🔥 INI KUNCI
    });
    ...
    $(document).ready(function () {
        $('#RelasiTable').DataTable({
            ...
            pageLength: 9,      // Menampilkan 9 data per halaman
            ...
            serverSide: true,
            deferRender: true,
            ajax: "{{ route('harmonispeserta.table') }}",
            columns: [
                { data: "DT_RowIndex" },
                { data: "judul_acara" },
    ...
    // Ambil Kegiatan Relasi Harmonis
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var i = 0; i < response.data.length; i++) {
                ...
                if (item.kategori != 455) continue; // relasi harmonis
    ...
    document.addEventListener("DOMContentLoaded", () => {
        const form = document.getElementById("form_relasi_peserta");
    ...
    Page.Batal = function()
    {
        $.ajax({
            type: 'GET',
            beforeSend: function() {
                $('#tambahRelasi').modal('hide');
                $('#detailRelasi').modal('hide');
                $('#editRelasi').modal('hide');
                $('#importRelasi').modal('hide');
                $('#Loader').modal('show');
            },
                ...
                success: function(response){
                    ...
                    if (validGolongan.includes(response.data.golongan)) {
                        ...
                        $('#e_gol_bimwin').select2({
                            ...
                            placeholder: 'Pilih',
                            minimumResultsForSearch: 0,
                            dropdownParent: $('#editRelasi') // 🔥 INI KUNCI
                        });
                    }
    ...
    document.addEventListener("DOMContentLoaded", () => {
        const form = document.getElementById("form_edit_relasi_peserta");
        if (!form) return;
        ...
        form.addEventListener("submit", function (e) {
            ...
            const modalEl = document.getElementById("editRelasi");
