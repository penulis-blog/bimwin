<script type="text/javascript">
    ...
    Page.Form = function(val)
    {
        if(val == '' || val == null || val == 0){
            ...
            $.ajax({
                url: "{{ url('b90ab757-1d41-4cf9-a0b1-3ba9dec45060') }}/" + val,
                type: 'GET',
    ...
    function createTable(data)
    {
        ...
        data.forEach(function(row, i){
            html += '<tr>';
            html += '<input type="hidden" name="id[]" value="'+row.public_id+'"/>';
            html += '<input type="hidden" name="id_roles[]" value="'+row.idpublic_dtinput+'"/>';
            html += '<input type="hidden" name="id_roles[]" value="'+row.idmst_input+'"/>';
            html += '<td>'+(i+1)+'.</td>';
            html += '<td style="text-align: left !important;">'+row.i_label+'</td>';
            html += '<td style="text-align: center !important;">'+row.placeholder+'</td>';
            html += '<td style="text-align: center !important;">'+row.i_type+'</td>';
            html += '<td style="text-align: center !important;">'+row.i_name+'</td>';
            html += '<td style="text-align: center !important;">'+row.i_event+'</td>';
            html += '<td style="text-align: center !important;">'+row.opsi_select+'</td>';
            html += '<td style="margin-top:-7px; margin-left:10px;"><input type="checkbox" name="is_tampilkan[]" id="istampilkan'+i+'" value="'+row.is_view+'"/>' + '</td>';
            html += '</tr>';
        });

        $('#permission_data_form').html(html);
        $('#FormConfig').modal('show');
        formchek(data);
    }
    ...
    document.addEventListener("DOMContentLoaded", () => {
        const form = document.getElementById("laporan_jadwalbrus");
            ...
            form.addEventListener("submit", async (e) => {
                ...
                const fields = {
                    l_acara: { value: l_acara, message: "Maaf, jenis acara harus dipilih.", skipIf: 'Pilih' },
                    l_kategori: { value: l_kategori, message: "Maaf, kategori laporan harus dipilih.", skipIf: 'Pilih' },
            ...
            try {
                ...
                // periksa apakah response tipe file (application/vnd.openxmlformats)
                const contentType = response.headers.get("content-type");

                if (contentType && contentType.includes("application/vnd.openxmlformats")) {
                    // ini file excel dari spatie
                    const blob = await response.blob();
                    const url = window.URL.createObjectURL(blob);
                    const a = document.createElement("a");
                    a.href = url;
                    a.download = "laporan-brus.xlsx"; // nama file hasil download
                    document.body.appendChild(a);
                    a.click();
                    a.remove();
                    window.URL.revokeObjectURL(url);
                    Swal.fire("Berhasil", "Laporan berhasil diunduh.", "success").then(() => {
                        window.location.href = window.location.pathname;
                    }); 
                } else {
                    const result = await response.json();
                    if (result.message === 201) {
                        Swal.fire("Error", "Maaf, data belum tersedia.", "error");
                        showModal("laporanJadwalBrus");
                    } else {
    ...
    // Input Data
    document.addEventListener("DOMContentLoaded", () => {
        const form = document.getElementById("form_jadwallainnya");
            ...
            form.addEventListener("submit", async (e) => {
                ...
                const fields = {
                    _kategori: { value: _kategori, message: "Maaf, kategori kegiatan harus dipilih.", skipIf: 'Pilih' },
    ...
    Page.Batal = function()
    {
        $.ajax({
            type: 'GET',
            beforeSend: function() {
                $('#tambahJadwalLain').modal('hide');
                $('#detailJadwalLain').modal('hide');
                $('#editJadwalLain').modal('hide');
                $('#FormulirJadwalLain').modal('hide');
                $('#laporanJadwalLain').modal('hide');
                $('#Loader').modal('show');
            },
    ...
    // Edit Data
    document.addEventListener("DOMContentLoaded", () => {
        const form = document.getElementById("form_edit_jadwallainnya");
    ...
    Page.Back = function(val) {
        ...
        if (val == '' || val == null || val == 0) {
            let baseUrl = "{{ route('barcodebrus.download', ':id') }}";
            let index = 1;
