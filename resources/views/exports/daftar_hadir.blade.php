...
<body>
    <table style="border-collapse: collapse; width: 100%; vertical-align: middle;">
        {{-- === HEADER === --}}
        <tr>
            <td colspan="8" style="text-align: center; font-weight: bold; font-size: 26pt;">
                DAFTAR HADIR
            </td>
        </tr>
        <tr>
            <td colspan="8" style="text-align: center; font-weight: bold; font-size: 24pt;">
                {{ strtoupper($single->judul_acara) }}
            </td>
        </tr>
        <tr>
            <td colspan="8" style="text-align: center; font-size: 20pt;">
                {{ ucwords($single->tempat) }}, {{ indo_periode($single->in, $single->out) }}
            </td>
        </tr>
        <tr><td colspan="6"></td></tr>
        <tr>
            <td colspan="2" style="font-size: 12pt; font-weight:bold;"><h3>{{ $single->tanggal_kegiatan }}</h3></td>
        </tr>
        {{-- === HEADER TABEL === --}}
        <tr style="font-weight: bold; text-align: center; font-size: 15px;">
            ...
            <td style="border: 1px solid black; font-weight:bold;">KABUPATEN/KOTA</td>
            <td style="border: 1px solid black; font-weight:bold;">KUA/UTUSAN</td>
        ...
        @foreach($data as $item)
            <tr style="font-size: 15px;">
                ...
                <td style="border:1px solid black; text-align:center; vertical-align: middle;">{{ strtoupper($item->provinsi ?? '-') }}</td>
                <td style="border:1px solid black; text-align:center; vertical-align: middle;">{{ strtoupper($item->kabupaten ?? '-') }}</td>
